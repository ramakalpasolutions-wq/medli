...
export async function generateInvoicePDF(data) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const baseFee = Number(data.baseFee || 0)
      const couponDiscount = Number(data.couponDiscount || 0)
      const discountedFee =
        data.discountedFee != null ? Number(data.discountedFee) : baseFee - couponDiscount

      const platformFee = Number(data.platformFee || 0)
      const gst = Number(data.gst || 0)
      const adminCouponDiscount = Number(data.adminCouponDiscount || 0)

      const serviceChargesRaw = platformFee + gst
      const serviceCharges = Math.max(0, serviceChargesRaw - adminCouponDiscount)
      ...
      if (data.gstDetails?.providerGstin) {
        ...
        providerLines.push(`GSTIN: ${data.gstDetails?.medliGstin || process.env.MEDLI_GSTIN}`)
      }
      ...
      doc.text('AMOUNT', TABLE_X + DESC_W + QTY_W + RATE_W, y + 10, {
        width: AMT_W - 12,
        align: 'right',
      })
      y += ROW_H
          ...
          : [
              {
                ...
                quantity: 1,
                rate: discountedFee || baseFee || 0,
                amount: discountedFee || baseFee || 0,
              },
            ]
      ...
      items.forEach((item, i) => {
        ...
        doc
          ...
          .text(String(item.quantity || 1), TABLE_X + DESC_W, y + 9, {
            width: QTY_W,
            align: 'center',
          })
          .text(formatAmount(item.rate), TABLE_X + DESC_W + QTY_W, y + 9, {
            width: RATE_W,
            align: 'right',
          })
      ...
      // ════════════════════════════════════════════════════════════════════════
      const SUM_X = TABLE_X + CONTENT * 0.5
      const SUM_W = CONTENT * 0.5
      const LABEL_W = SUM_W * 0.58
      const VALUE_W = SUM_W * 0.42

      const summaryLines = []

      summaryLines.push({ label: 'Base Fee', value: formatAmount(baseFee) })
      ...
      if (discountedFee !== baseFee) {
        summaryLines.push({
          ...
          value: formatAmount(discountedFee),
        })
      }

      if (serviceCharges > 0) {
        summaryLines.push({
          label: 'Service Charges (incl. GST)',
          value: formatAmount(serviceCharges),
        })
      }

      if (serviceChargesRaw > 0 && adminCouponDiscount > 0) {
        summaryLines.push({
          label: 'Platform Coupon Discount',
          value: `- ${formatAmount(adminCouponDiscount)}`,
          color: COLORS.success,
        })
      }
      ...
      if (pgRef) {
        ...
        doc.font('Helvetica').fontSize(7).fillColor(COLORS.muted).text('Payment Gateway:', MARGIN + 12, y + 6)
        doc
          .font('Helvetica-Bold')
          .fontSize(7)
          .fillColor(COLORS.dark)
        ...
        doc.font('Helvetica').fontSize(7).fillColor(COLORS.muted).text('Reference:', MARGIN + 12, y + 16)
        doc.font('Helvetica-Bold').fontSize(7).fillColor(COLORS.dark).text(pgRef, MARGIN + 60, y + 16)
      ...
      if (data.gstDetails?.hsnCode || data.gstDetails?.gstRate) {
        ...
        if (data.gstDetails.hsnCode) {
          doc.font('Helvetica').fontSize(7).fillColor(COLORS.muted).text('HSN/SAC:', gstX, y + 9)
          doc
            .font('Helvetica-Bold')
            .fontSize(7)
            .fillColor(COLORS.dark)
        ...
        if (data.gstDetails.medliGstin) {
          doc.font('Helvetica').fontSize(7).fillColor(COLORS.muted).text('MEDLI GSTIN:', gstX, y + 9)
          doc
            .font('Helvetica-Bold')
            .fontSize(7)
            .fillColor(COLORS.dark)
