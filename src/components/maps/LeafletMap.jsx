...
export default function LeafletMap({
  ...
  const fetchUserLocation = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords: { latitude: lat, longitude: lng } }) => {
        ...
        const dist = {}
        markers.forEach((m) => {
          if (m.lat && m.lng) dist[m.id] = calcDistance(lat, lng, m.lat, m.lng)
        })
        setDistances(dist)
      ...
      (err) => {
        ...
        setLocError(
          ...
          err.code === 2 ? 'Location unavailable' : 'Timeout'
        )
      },
  ...
  useEffect(() => {
    ...
    markers.forEach((m) => {
      if (m.lat && m.lng)
        dist[m.id] = calcDistance(userLocation.lat, userLocation.lng, m.lat, m.lng)
    })
    setDistances(dist)
