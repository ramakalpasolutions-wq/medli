...
function AuthInput({ label, icon: Icon, rightElement, ...props }) {
  const [focused, setFocused] = useState(false)
...
function LoginTabBtn({ label, icon: Icon, active, onClick }) {
  const [h, setH] = useState(false)
...
function LoginContent() {
  ...
  const TABS = [
    ...
    { key: 'email_pass', label: 'Password', icon: KeyRound },
  ]
              ...
              <div style={{ animation: 'auth-slide-in .2s ease' }}>
                {!otpSent ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    ...
                    <AuthBtn onClick={sendOtp} loading={sending} disabled={phone.length !== 10 || sending}>
                      Send OTP
