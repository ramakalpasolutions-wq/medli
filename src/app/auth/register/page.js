// C:\Users\ASUS\medli2\src\app\auth\register\page.js
'use client'
  ...
  const handlePaste = (e) => {
    ...
    if (pasted) {
      ...
      document.getElementById(
        `${idPrefix}-${Math.min(pasted.length, 5)}`
      )?.focus()
    }
  }
...
/* ─── Auth Input ─────────────────────────────────────────────────────── */
function AuthInput({ label, icon, hint, rightElement, error: fieldError, ...props }) {
  const [focused, setFocused] = useState(false)
  return (
      ...
      <div style={{ position: 'relative' }}>
        {icon && (
          <span style={{
            ...
            color: fieldError
              ? '#fca5a5'
              : focused ? '#818cf8' : 'rgba(255,255,255,0.35)',
...
function TermsCheckbox({ checked, onChange, error: fieldError }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
        ...
        <div style={{ position: 'relative', flexShrink: 0, marginTop: 1 }}>
          <input
            ...
            style={{
              ...
              width: 18, height: 18, margin: 0, cursor: 'pointer',
            }}
          />
          <div style={{
            ...
            border: `2px solid ${
              fieldError
                ? 'rgba(239,68,68,0.6)'
                : checked ? '#6366f1' : 'rgba(255,255,255,0.3)'
            }`,
            background: checked
              ? 'linear-gradient(135deg,#6366f1,#8b5cf6)'
              : 'transparent',
      ...
      {fieldError && (
        <p style={{ fontSize: 11, color: '#fca5a5', margin: 0, paddingLeft: 4 }}>
          ⚠ {fieldError}
        </p>
      )}
    </div>
...
function StepDot({ icon, label, done, active }) {
  const bg    = done
    ...
    : active ? 'rgba(99,102,241,0.2)' : 'rgba(255,255,255,0.06)'
  const color = done
    ? '#10b981'
    : active ? '#818cf8' : 'rgba(255,255,255,0.3)'
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <span style={{ color, fontWeight: done || active ? 600 : 400 }}>
        {label}
      </span>
    </div>
  )
...
function RegisterContent() {
  ...
  /* Progress: 4 steps — 1:Info, 2:Phone OTP, 3:Email OTP, 4:Confirm */
  ...
  /* ── Step 1 → validate all fields → send phone OTP ── */
    ...
    try {
      const res  = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({
          ...
          email:         email.trim().toLowerCase(),
          password,
          agreedToTerms: true,          // ✅ saved in DB
  ...
  /* ── Field change helpers that clear field errors ── */
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            {/* ══════════════════════════════════════
                STEP 1 — Details (all mandatory)
            ══════════════════════════════════════ */}
            {step === 1 && (
              <div style={{ animation: 'auth-step .2s ease' }}>
                <h2 style={{
                  fontSize: 18, fontWeight: 800, color: '#fff', margin: '0 0 4px',
                }}>
                  Create account
                </h2>
                <p style={{
                  fontSize: 13, color: 'rgba(255,255,255,0.4)', margin: '0 0 6px',
                }}>
                ...
                <p style={{
                  ...
                  margin: '0 0 18px', display: 'flex', alignItems: 'center', gap: 4,
                }}>
                  <span style={{ color: '#f87171', fontSize: 13 }}>*</span>
                  Required field
                </p>
            ...
            {step === 2 && (
              <div style={{ animation: 'auth-step .2s ease' }}>
                ...
                <p style={{
                  fontSize: 13, color: 'rgba(255,255,255,0.4)', margin: '0 0 20px',
                }}>
                  OTP sent to{' '}
                ...
                <div style={{
                  display: 'flex', flexDirection: 'column', gap: 16, marginTop: 20,
                }}>
                  <p style={{
            ...
            {step === 3 && (
              <div style={{ animation: 'auth-step .2s ease' }}>
                <GhostBtn
                  onClick={() => { setStep(2); setEmailOtp(''); setError('') }}
                >
                  ← Back
                </GhostBtn>
                ...
                <div style={{
                  ...
                  <ResendRow
                    ...
                    sending={sending}
                    onBack={() => { setStep(2); setEmailOtp(''); setError('') }}
                    backLabel="← Back"
                  />
                </div>
              </div>
              ...
              <div style={{ animation: 'auth-step .2s ease' }}>
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {phoneVerified && (
                      <VerifiedBadge
                        icon="✓" label="Phone verified" color="#10b981"
                      />
                    )}
                    {emailVerified && (
                      <VerifiedBadge
                        icon="✓" label="Email verified" color="#10b981"
                      />
                    )}
                    <VerifiedBadge icon="🔒" label="Password set" color="#6366f1" />
                    <VerifiedBadge icon="📋" label="Terms accepted" color="#10b981" />
                  </div>
                </div>
