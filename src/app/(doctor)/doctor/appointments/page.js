...
function ViewToggle({ view, onChange }) {
  return (
    <div style={{ display: 'flex', gap: 3, background: '#f1f5f9', borderRadius: 10, padding: 3 }}>
      {[
        { key: 'strip', label: 'Strip', icon: <LayoutGrid size={14} strokeWidth={2.3} /> },
        { key: 'calendar', label: 'Month', icon: <CalendarDays size={14} strokeWidth={2.3} /> },
        { key: 'list', label: 'List', icon: <List size={14} strokeWidth={2.3} /> },
      ].map((v) => (
        <button
          key={v.key}
        ...
        return (
          <button
            ...
            style={{
              ...
              padding: '8px 6px',
              borderRadius: 12,
              border: selected ? 'none' : isToday ? '1.5px solid rgba(99,102,241,0.2)' : '1.5px solid transparent',
              flexShrink: 0,
              cursor: 'pointer',
              minHeight: 60,
              background: selected
    ...
    >
      ...
      >
        ...
        <div style={{ textAlign: 'center' }}>
          <p style={{ fontSize: 16, fontWeight: 800, color: '#0f172a', margin: 0 }}>{MONTHS[month]}</p>
              ...
              style={{
                ...
                background: isSelected
                  ...
                  : isToday
                    ? 'rgba(99,102,241,0.08)'
                    : 'transparent',
                color: isSelected ? '#fff' : !current ? '#d1d5db' : isToday ? '#6366f1' : '#1e293b',
        ...
        return (
          <div key={dateStr}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
              <div
                style={{
                  background: isToday ? 'linear-gradient(135deg,#6366f1,#8b5cf6)' : '#f1f5f9',
...
function ApptCard({ b, mounted, idx, onClick }) {
  ...
  const diffMins = now ? (new Date(b.startTime) - now) / 60000 : null
  const showJoin =
    b.type === 'online' && b.meetLink && diffMins !== null && diffMins <= 15 && diffMins >= -30
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
          <div
            style={{
              ...
              marginBottom: 14,
              background:
                bk.status === 'confirmed' ? '#eff6ff' : bk.status === 'completed' ? '#f0fdf4' : '#fff1f2',
              border: `1px solid ${
                bk.status === 'confirmed' ? '#bfdbfe' : bk.status === 'completed' ? '#bbf7d0' : '#fecaca'
              }`,
              display: 'flex',
              alignItems: 'center',
              gap: 8,
          ...
          >
            <span style={{ color: bk.status === 'confirmed' ? '#2563eb' : bk.status === 'completed' ? '#16a34a' : '#ef4444' }}>
              {bk.status === 'confirmed' ? (
...
export default function AppointmentsPage() {
  ...
  const { data: dayData, isLoading: dayLoading, mutate: dayMutate } = useSWR(
    selectedDate && view !== 'list' ? `/api/bookings?limit=50&dateFrom=${selectedDate}&dateTo=${selectedDate}` : null,
    fetcher
  )
  ...
  const { data: monthData } = useSWR(
    view === 'calendar' ? `/api/bookings?limit=200&dateFrom=${monthFrom}&dateTo=${monthTo}` : null,
    fetcher
  )
  ...
  const { data: listData, isLoading: listLoading, mutate: listMutate } = useSWR(
    view === 'list' ? `/api/bookings?limit=200&dateFrom=${listFrom}&dateTo=${listTo}` : null,
    fetcher
  )
  ...
  const updateStatus = async (id, status) => {
    try {
      ...
      const json = await res.json()
      json.success ? toast.success(`Marked ${status.replace('_', ' ')}`) : toast.error(json.error)
      mutate()
  ...
  return (
    <>
      ...
      <AdminHeader
        ...
        subtitle={
          view === 'list'
            ...
            : selectedDate
              ? `${filtered.length} appointment${filtered.length !== 1 ? 's' : ''} · ${new Date(
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        {[
          { key: 'all', label: 'All', icon: <CalendarDays size={13} strokeWidth={2.3} /> },
          { key: 'hospital', label: 'In-Person', icon: <Building2 size={13} strokeWidth={2.3} /> },
          { key: 'online', label: 'Online', icon: <Video size={13} strokeWidth={2.3} /> },
        ].map((f) => (
          <FilterPill
            key={f.key}
            label={f.label}
        ...
        <span style={{ fontSize: 12, color: '#94a3b8', marginLeft: 4 }}>{totalShown} shown</span>
      </div>

      {view === 'strip' && (
        <>
          {mounted && dates.length > 0 ? (
            <StripView selectedDate={selectedDate} setSelectedDate={setSelectedDate} todayStr={todayStr} dates={dates} />
          ) : (
          ...
          {selectedDate && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                <div
                  style={{
                    background: selectedDate === todayStr ? 'linear-gradient(135deg,#6366f1,#8b5cf6)' : '#f1f5f9',
