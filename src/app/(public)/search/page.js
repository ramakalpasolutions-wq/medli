...
/* ─── Constants ──────────────────────────────────────────────────────── */
const TABS = [
  { key: 'hospitals', label: 'Hospitals', emoji: '🏥', color: '#6366f1' },
  { key: 'labs',      label: 'Labs',      emoji: '🧪', color: '#10b981' },
  { key: 'doctors',   label: 'Doctors',   emoji: '👨‍⚕️', color: '#8b5cf6' },
]
...
function TabBar({ tabs, active, onChange }) {
  return (
    <div style={{
      ...
      {/* ✅ Fixed: key is explicit, no spread of object containing key */}
      {tabs.map((t) => (
        <TabBtn
          key={t.key}
          label={t.label}
...
/* ─── Tab Button ─────────────────────────────────────────────────────── */
function TabBtn({ label, emoji, active, onClick }) {
...
function ViewToggle({ view, onChange, disabled }) {
  ...
  return (
    <div style={{
      ...
      {/* ✅ Fixed: explicit props, no color variable, no spread */}
      {VIEWS.map((v) => (
        <ViewBtn
          key={v.key}
          icon={v.icon}
...
function SearchContent() {
  ...
  /* ── Data fetching ── */
  const { data: hData, isLoading: hLoad } = useSWR(tab === 'hospitals' ? `/api/hospitals?${qs}` : null, fetcher)
  const { data: lData, isLoading: lLoad } = useSWR(tab === 'labs'      ? `/api/labs?${qs}`      : null, fetcher)
  const { data: dData, isLoading: dLoad } = useSWR(tab === 'doctors'   ? `/api/doctors?${qs}`   : null, fetcher)
  ...
  }, [tab, hospitals, labs, accentColor, activeTab])
  ...
  const clearFilters = () => { setQuery(''); setCity('') }
