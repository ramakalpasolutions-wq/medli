...
// FIX: fetcher returns j.data directly — booking API returns { data: { booking: {...} } }
const fetcher = (url) =>
...
export default function BookingSuccessPage({ params }) {
  ...
  // FIX: poll every 2s for up to 10s so callback has time to process
  const { data: booking, mutate } = useSWR(
    `/api/bookings/${id}`,
    fetcher,
    {
      refreshInterval: booking?.paymentStatus === 'paid' ? 0 : 2000,
  ...
  // FIX: verify payment if still pending — retry up to 5 times
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        await mutate()  // re-fetch booking after verify
        setRetryCount((c) => c + 1)
      } catch {}
    }, 1500)

    return () => clearTimeout(timer)
  }, [booking?.onePayTxnId, booking?.paymentStatus, retryCount])
      ...
      <div style={{
        ...
        <div style={{
          ...
          {booking ? (
            <div style={{
              ...
              {[
                ...
                ['Type', typeLabel(booking.type), false],
                ...
                <div key={k} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid rgba(16,185,129,0.1)' }}>
                  <span style={{ fontSize: 12, color: '#64748b' }}>{k}</span>
                  <span style={{ fontSize: 12, fontWeight: 600, color: '#334155', fontFamily: mono ? 'monospace' : undefined }}>{v}</span>
                </div>
              ))}
              <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 10, marginTop: 4, borderTop: '1px solid rgba(16,185,129,0.2)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, animation: 'suc-in .3s ease .5s both' }}>
            ...
            <ActionBtn variant="secondary" onClick={() => router.push('/user/bookings')}>
              My Bookings
            </ActionBtn>
