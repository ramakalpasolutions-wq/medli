      ...
      <div style={{ padding: '0 24px 24px' }}>
        ...
        >
          ...
          <p style={{ fontSize: 13, color: '#92400e', margin: 0, lineHeight: 1.6 }}>
            Transfer the exact amount to the account below using your bank. After the transfer,
            go to the <strong>Processing</strong> tab and confirm with the UTR number.
          </p>
        </div>
        ...
        <p style={{ fontSize: 12, color: '#64748b', textAlign: 'center', marginBottom: 16 }}>
          After transfer, go to <strong>Processing</strong> tab and <strong>Confirm with UTR</strong>
        </p>
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await fetch(`/api/settlements/${settlement.id}/confirm`, {
        ...
        credentials: 'include',
        body: JSON.stringify({ utrNumber: cleaned, transferMode }),
      })
          ...
          {[
            ...
            .map((row) => (
              <div key={row.l} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                ...
                <span style={{ fontWeight: row.bold ? 700 : 600, color: '#1e293b' }}>{row.v}</span>
              </div>
            ))}
        </div>
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <p style={{ fontSize: 13, fontWeight: 700, margin: 0, color: initiated ? '#64748b' : C.text }}>
            {entity.name}
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 16 }}>
        {[
          {
            label: 'Refunds',
            value: Number(entity.refundsDeducted) > 0 ? `- ${fmtRs(entity.refundsDeducted)}` : 'None',
            red: Number(entity.refundsDeducted) > 0,
          },
          {
            label: 'You Receive',
            value: fmtRs(entity.netSettlementAmount),
            green: true,
          },
        ].map((s) => (
          <div
          ...
          >
            <p style={{ fontSize: 11, marginBottom: 4, color: s.green ? '#16a34a' : '#64748b', fontWeight: s.green ? 600 : 400 }}>
              {s.label}
            </p>
            <p style={{ fontSize: 13, fontWeight: 700, margin: 0, color: s.green ? '#15803d' : s.red ? '#dc2626' : '#1e293b' }}>
              {s.value}
            </p>
          </div>
        ))}
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
          ...
          <span style={{ fontWeight: 600, color: entity.bankAccount?.isVerified ? '#16a34a' : '#d97706' }}>
        ...
        {entity.bankAccount?.bankName && (
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span style={{ fontWeight: 500, color: '#1e293b' }}>{entity.bankAccount.bankName}</span>
          </div>
        )}
      </div>
      ...
      {initiated ? (
        ...
        >
          ...
          <div style={{ textAlign: 'center' }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: '#92400e', margin: 0 }}>Settlement Initiated</p>
        ...
        <>
          ...
          {!entity.bankAccount?.isVerified && (
            <p style={{ fontSize: 11, color: '#d97706', textAlign: 'center', marginTop: 8 }}>
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
        <div>
          <p style={{ fontSize: 14, fontWeight: 600, color: '#1e293b', margin: '0 0 2px' }}>{settlement.entityName}</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
        <div style={{ background: '#f8fafc', borderRadius: 12, padding: '10px 14px' }}>
          ...
          <p style={{ fontSize: 15, fontWeight: 700, color: '#1e293b', margin: 0 }}>{fmtRs(settlement.netSettlementAmount)}</p>
        </div>

        <div style={{ background: '#f8fafc', borderRadius: 12, padding: '10px 14px' }}>
          <p style={{ fontSize: 11, color: '#64748b', marginBottom: 4 }}>Bookings</p>
          <p style={{ fontSize: 15, fontWeight: 700, color: '#1e293b', margin: 0 }}>{settlement.totalBookings}</p>
        </div>
      </div>
      ...
      >
        Transfer <strong>{fmtRs(settlement.netSettlementAmount)}</strong> to <strong>{settlement.beneficiaryName}</strong>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <Button variant="primary" size="sm" style={{ flex: 2 }} onClick={() => onConfirm(settlement)}>
          Confirm with UTR
        </Button>
      </div>
    </motion.div>
...
export default function SettlementsPage() {
  ...
  const { data: processingData, mutate: mutateProcessing } = useSWR(
    tab === 'processing' ? '/api/settlements?status=processing&limit=50' : null,
    fetcher,
...
const { data: history, isLoading: historyLoading, mutate: mutateHistory } = useSWR(
  ...
  useEffect(() => {
    if (tab === 'processing') setInitiatedIds(new Set())
  }, [tab])
  ...
  const handleCancelSettlement = async (settlement) => {
    const reason = prompt(`Cancel settlement ${settlement.settlementNumber}? Reason required (min 5 chars):`)
    ...
    try {
      ...
      const json = await res.json()
      if (json.success) toast.success('Settlement cancelled')
      ...
      console.error('[cancel-settlement]', err)
  ...
  const handleExportSettlements = async () => {
    try {
      ...
      const status = tab === 'failed' ? 'failed' : 'completed'
      const qs = new URLSearchParams({ status, from, to })

      const res = await fetch(`/api/settlements/export?${qs.toString()}`, {
        credentials: 'include',
      })
      ...
      const a = document.createElement('a')
      a.href = url
      a.download = `MEDLI-Settlements-${status}-${from}-to-${to}.csv`
      document.body.appendChild(a)
      a.click()
      a.remove()
      URL.revokeObjectURL(url)
      ...
      console.error('[settlements-export]', err)
  ...
  const handleMailSettlements = async () => {
    try {
      const { from, to } = getDateRangeParams(range)
      ...
      const res = await fetch('/api/settlements/export/mail', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      toast.success('Settlement CSV sent to email successfully')
    } catch (err) {
      console.error('[settlements-mail]', err)
  ...
  const historyCols = [
    ...
    },
    {
      ...
      render: (v, row) => (
        <div>
          <p style={{ fontSize: 13, fontWeight: 600, color: '#1e293b', margin: '0 0 3px' }}>{v}</p>
    ...
    { key: 'transferMode', header: 'Mode', render: (v) => v || '—' },
    {
      key: 'status',
      header: 'Status',
      render: (v) => (
        <Badge variant={v === 'completed' ? 'success' : v === 'failed' ? 'danger' : 'warning'} size="sm" dot>
          {v}
        </Badge>
      ),
    },
    {
      ...
      header: 'Date',
      render: (v) =>
        mounted && v ? new Date(v).toLocaleDateString('en-IN', { dateStyle: 'medium' }) : '—',
    },
    {
      key: 'actions',
      header: '',
    ...
    <div>
      ...
      {tab === 'pending' && (
        <div>
          ...
          >
            <StatsCard title="Pending Entities" value={notYetInitiated.length} icon={Clock} color="orange" />
            <StatsCard title="Processing" value={processingSttlmnts.length} icon={Banknote} color="blue" />
          </div>
          ...
          >
            <div>
              ...
              <p style={{ fontSize: 14, fontWeight: 600, color: '#1e293b', margin: 0 }}>Initiate all pending</p>
            </div>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <p style={{ fontSize: 13, color: '#92400e', margin: 0 }}>
                  <strong>{initiatedIds.size}</strong> settlement{initiatedIds.size > 1 ? 's' : ''} initiated. Go to{' '}
                  <strong>Processing</strong> tab to confirm with UTR numbers.
                </p>
              </div>
          ...
          {pendingLoading ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(320px,1fr))', gap: 16 }}>
              {[1, 2].map((i) => (
                <div key={i} style={{ background: '#f8fafc', borderRadius: 20, height: 280, opacity: 0.6 }} />
              ))}
            </div>
            ...
            <Card>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '32px 24px' }}>
                ...
                <div>
                  <p style={{ fontSize: 14, fontWeight: 700, color: '#065f46', margin: '0 0 4px' }}>All settled!</p>
                  <p style={{ fontSize: 12, color: '#94a3b8', margin: 0 }}>No pending settlements at this time.</p>
                </div>
              </div>
            </Card>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(320px,1fr))', gap: 16 }}>
              {pendingEntities.map((entity, i) => (
                <motion.div key={entity.entityId} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.06 }}>
                  <PendingEntityCard
                    entity={entity}
          ...
          {processingSttlmnts.length === 0 ? (
            <Card>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '32px 24px' }}>
                ...
                <div>
                  <p style={{ fontSize: 14, fontWeight: 600, color: '#374151', margin: '0 0 4px' }}>No processing settlements</p>
      ...
      {(tab === 'history' || tab === 'failed') && (
        <div>
          ...
          <DataTable
            ...
            emptyTitle={tab === 'failed' ? 'No failed settlements' : 'No completed settlements'}
            keyField="id"
          />
        </div>
      )}
