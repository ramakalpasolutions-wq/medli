...
function StatusBreakdown({ data }) {
  ...
  if (!data.length) return (
    <p style={{ fontSize: 13, color: '#94a3b8', textAlign: 'center', padding: '24px 0' }}>No booking data yet</p>
  )
  return (
  ...
  return (
    <a href={href} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{
        ...
        textDecoration: 'none', transition: 'all .14s ease',
        textAlign: 'left',
      }}>
      {label}
    </a>
...
function ViewAllLink({ href }) {
  ...
  return (
    <a href={href} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{ fontSize: 12, fontWeight: 600, color: h ? '#4f46e5' : '#6366f1', textDecoration: 'none', transition: 'color .14s ease', display: 'flex', alignItems: 'center', gap: 3 }}>
      View all ›
    </a>
  )
}
...
export default function SuperAdminDashboard() {
  ...
  return (
    <>
      ...
      {/* Main stats */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 14, marginBottom: 20, animation: 'sd-in .3s ease' }}>
        {[
          { title: 'Total Users',   value: o.totalUsers    || 0, icon: '👥', color: 'blue',   href: '/super-admin/users'     },
          { title: 'Hospitals',     value: o.totalHospitals|| 0, icon: '🏥', color: 'purple', href: '/super-admin/hospitals' },
          { title: 'Labs',          value: o.totalLabs     || 0, icon: '🧪', color: 'green',  href: '/super-admin/labs'      },
          { title: 'Doctors',       value: o.totalDoctors  || 0, icon: '👨‍⚕️', color: 'orange', href: '/super-admin/doctors'   },
        ].map((s) => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 14, marginBottom: 20 }}>
        <StatsCard title="Today's Bookings" value={t.bookings || 0} icon="📅" color="indigo" />
        <StatsCard title="Today's Revenue"  value={`₹${Number(t.revenue || 0).toLocaleString('en-IN')}`} icon="💰" color="green" />
        <StatsCard title="Pending Alerts"   value={totalAlerts} icon="⚠️" color="red" />
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <SCard title="Platform Summary">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <SummaryItem label="Total Bookings"  value={o.totalBookings     || 0} icon="📅" />
              <SummaryItem label="Confirmed"       value={o.confirmedBookings || 0} icon="✅" />
              <SummaryItem label="Pending Refunds" value={a.pendingRefunds    || 0} icon="↩️" />
              <SummaryItem label="Pending Settle"  value={a.pendingSettlements|| 0} icon="⏳" />
            </div>
          </SCard>
