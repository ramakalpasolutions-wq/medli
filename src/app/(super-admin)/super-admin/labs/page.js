...
export default function LabsPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'Active',
      render: (v) => (
        <Badge variant={v ? 'success' : 'neutral'} size="sm">
          {v ? 'Active' : 'Inactive'}
        </Badge>
      ),
    },
    ...
    },
    {
      ...
      render: (_, row) => (
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          ...
          <ABtn label="✏️ Edit" variant="outline" onClick={() => setEditItem(row)} />
          {!row.isApproved && (
            <ABtn
              label="✓ Approve"
              variant="success"
              onClick={() => setAction({ type: 'approve', lab: row })}
            />
          )}
          <ABtn
            label={row.isActive ? 'Disable' : 'Enable'}
            variant={row.isActive ? 'danger' : 'outline'}
            onClick={() => setAction({ type: 'toggle', lab: row })}
          />
        </div>
      ),
    },
  ...
  return (
    <>
      ...
      <div>
        ...
        <Modal open={!!editItem} onClose={() => setEditItem(null)} title="Edit Lab" size="lg">
          {editItem && (
            <EditLabForm
              lab={editItem}
      ...
      >
        ...
        >
          ...
          <FormInput
            ...
            onChange={(e) =>
              setForm((f) => ({ ...f, address: { ...f.address, pinCode: e.target.value } }))
            }
            placeholder="500001"
          />
          ...
          <ToggleRow
            label="Enable home sample collection?"
            checked={form.homeCollection.enabled}
            onChange={(v) =>
              setForm((f) => ({ ...f, homeCollection: { ...f.homeCollection, enabled: v } }))
            }
          />
          ...
          {form.homeCollection.enabled && (
            <FormInput
              ...
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  homeCollection: { ...f.homeCollection, areaCoverage: e.target.value },
                }))
              }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      >
        ...
        <InfoBox
          label="📅 Added"
          value={new Date(l.createdAt).toLocaleDateString('en-IN', { dateStyle: 'long' })}
        />
      </div>
...
function EditLabForm({ lab, onClose, onSaved }) {
  const toast = useToast()
  const [saving, setSaving] = useState(false)
  const [form, setForm] = useState({
    name: lab.name || '',
    ...
    platformFeePercent: lab.platformFeePercent ?? 8,
    address: {
    ...
    homeCollection: {
      enabled: !!lab.homeCollection?.enabled,
      areaCoverage: Array.isArray(lab.homeCollection?.areaCoverage)
        ? lab.homeCollection.areaCoverage.join(', ')
        : '',
    },
    location: {
      lat:
        Array.isArray(lab.location?.coordinates) && lab.location.coordinates[1] != null
          ? String(lab.location.coordinates[1])
          : '',
      lng:
        Array.isArray(lab.location?.coordinates) && lab.location.coordinates[0] != null
          ? String(lab.location.coordinates[0])
          : '',
    },
  })
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (!res.ok || !json?.success) {
        toast.error(json?.error || 'Failed to update lab')
        return
      }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 8 }}>
        <SecondaryBtn onClick={onClose}>Cancel</SecondaryBtn>
        <SaveBtn onClick={handleUpdate} loading={saving} label="💾 Update Lab" />
      </div>
    </div>
  )
}
