  ...
  return (
    <button onClick={onClick} disabled={isLoading} onMouseEnter={() => !isLoading && setH(true)} onMouseLeave={() => setH(false)}
      style={{
        ...
        color:h?'#10b981':'#94a3b8', fontSize:12, fontWeight:600,
        cursor:isLoading?'not-allowed':'pointer', transition:'all .12s ease', opacity:isLoading?.6:1,
      }}>
      {isLoading ? <span style={{ width:12,height:12,borderRadius:'50%',border:'2px solid #94a3b8',borderTopColor:'transparent',animation:'ls-spin .7s linear infinite',display:'inline-block' }} /> : '⬇'}
      PDF
    </button>
  )
}
...
function MBtns({ onCancel, onSave, saving }) {
  ...
  return (
    <div style={{ display:'flex', gap:10, marginTop:20 }}>
      <button onClick={onCancel} style={{ flex:1, padding:'11px', borderRadius:12, border:'1.5px solid #e2e8f0', background:'#fff', color:'#475569', fontSize:13, fontWeight:600, cursor:'pointer' }}>Cancel</button>
      <button onClick={onSave} disabled={saving} onMouseEnter={() => !saving && setH(true)} onMouseLeave={() => setH(false)}
        style={{
          flex:1, padding:'11px', borderRadius:12, border:'none',
          background:saving?'#e2e8f0':h?'linear-gradient(135deg,#059669,#047857)':'linear-gradient(135deg,#10b981,#059669)',
        ...
        {saving && <span style={{ width:14,height:14,borderRadius:'50%',border:'2px solid rgba(255,255,255,0.4)',borderTopColor:'#fff',animation:'ls-spin .7s linear infinite',display:'inline-block' }} />}
        Save Bank Account
      </button>
    </div>
  )
...
export default function LabSettlements() {
  ...
  const { data: historyData, isLoading } = useSWR(
    labId ? `/api/settlements?page=${page}&limit=10&entityType=lab&entityId=${labId}` : null,
    fetcher
  )
  ...
  const cols = [
    ...
    { key:'grossAmount',      header:'Test Revenue', render:(v) => <span style={{ fontSize:13 }}>₹{Number(v||0).toLocaleString('en-IN')}</span> },
  ...
  return (
    <>
      <style>{KF}</style>
      <AdminHeader title="Settlements" subtitle="Your test revenue settlements"
        breadcrumbs={[{label:'Lab Admin'},{label:'Settlements'}]}
        actions={<BankBtn onClick={() => setBankOpen(true)} />}
      />

      {/* Info banner */}
      <div style={{ display:'flex', alignItems:'flex-start', gap:10, background:'rgba(16,185,129,0.06)', border:'1px solid rgba(16,185,129,0.15)', borderRadius:14, padding:'12px 16px', marginBottom:20 }}>
      ...
      {myPending ? (
        <div style={{
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:18 }}>
            <div>
              <p style={{ fontSize:11, fontWeight:700, color:'#10b981', letterSpacing:'1px', textTransform:'uppercase', margin:'0 0 4px' }}>Pending Settlement</p>
            ...
            <div style={{ width:44, height:44, background:'rgba(16,185,129,0.12)', borderRadius:12, display:'flex', alignItems:'center', justifyContent:'center', fontSize:22 }}>⏳</div>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:12, marginBottom:16 }}>
            {[
              { label:'Test Revenue',       value:`₹${Number(myPending.grossAmount||0).toLocaleString('en-IN')}`,           sub:`${myPending.totalBookings} bookings`, highlight:false },
              { label:'Refunds Deducted',   value:myPending.refundsDeducted>0?`- ₹${Number(myPending.refundsDeducted).toLocaleString('en-IN')}`:'None', sub:'Cancelled bookings', red:myPending.refundsDeducted>0 },
              ...
              <div key={label} style={{ background:'rgba(255,255,255,0.7)', borderRadius:14, padding:14, border:highlight?'2px solid rgba(16,185,129,0.3)':'none' }}>
          ...
          <p style={{ fontSize:12, color:'#10b981', margin:0 }}>💡 Contact support@medli.in if your settlement is overdue.</p>
        </div>
      ) : pendingData && (
      ...
      <div style={{ background:'#fff', borderRadius:20, border:'1px solid #f1f5f9', overflow:'hidden', boxShadow:'0 2px 8px rgba(0,0,0,0.04)' }}>
        ...
        <div style={{ padding:0 }}>
          {!labId ? (
            <div style={{ padding:32 }}><EmptyState title="Loading..." message="Fetching lab data" /></div>
          ) : (
