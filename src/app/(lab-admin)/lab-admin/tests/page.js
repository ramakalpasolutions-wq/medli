  ...
  return (
    ...
    >
      ...
      >
        ...
        {test.category && (
          <Badge variant="info" size="sm" style={{ marginLeft: 8, flexShrink: 0 }}>
            {test.category}
          </Badge>
        )}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 12 }}>
        {test.sampleType && (
          ...
          >
            <TestTube2 size={12} strokeWidth={2.3} />
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {test.discountedPrice ? (
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
              <span style={{ fontSize: 14, fontWeight: 800, color: '#1e293b' }}>
                ₹{Number(test.discountedPrice).toLocaleString('en-IN')}
              </span>
              <span
                style={{
                  fontSize: 11,
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          <TestRowBtn
            label="Edit"
            color="blue"
            icon={<PencilLine size={13} strokeWidth={2.3} />}
            onClick={onEdit}
          />
          <TestRowBtn
            label="Remove"
...
function TestRowBtn({ label, color, onClick, icon }) {
  const [h, setH] = useState(false)
...
function SaveTestBtn({ label, onClick, loading: isLoading }) {
  ...
  return (
    ...
    >
      {isLoading && (
        <Loader2 size={14} strokeWidth={2.4} style={{ animation: 'lt-spin .7s linear infinite' }} />
      )}
      {label}
    </button>
  )
    ...
    try {
      const payload = {
        ...
        turnaroundTime: form.turnaroundTime.value
          ? {
              value: Number(form.turnaroundTime.value),
              unit: form.turnaroundTime.unit,
            }
          : undefined,
      ...
      if (json.success) {
        toast.success(editTest ? 'Test updated' : 'Test added')
        setPanelOpen(false)
        mutate()
      } else {
  ...
  return (
    <>
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon={<FlaskConical size={48} strokeWidth={1.8} />}
      ...
      {panelOpen && (
        <>
          ...
          >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <FInput
                  ...
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      turnaroundTime: { ...f.turnaroundTime, value: e.target.value },
                    }))
                  }
                  placeholder="24"
                />
                <FSelect
                  ...
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      turnaroundTime: { ...f.turnaroundTime, unit: e.target.value },
                    }))
                  }
                >
            ...
            >
              <SaveTestBtn
                label={editTest ? 'Update Test' : 'Add Test'}
                onClick={handleSave}
                loading={saving}
              />
            </div>
