// C:\Users\ASUS\medli2\src\app\(lab-admin)\lab-admin\dashboard\page.js
'use client'
...
function Skeleton({ h = 80 }) {
  return (
    <div style={{ height: h, borderRadius: 14, ...SHIMMER }} />
  )
}
...
function TodayRow({ b, mounted }) {
  return (
    <div className="db-today-row">
      ...
      <Badge
        variant={getStatusVariant(b.labStatus || 'pending')}
        size="sm"
      >
...
export default function LabAdminDashboard() {
  ...
  const homeCollections = todayBookings.filter(
    (b) => b.collectionType === 'home'
  )
  const reportsReady    = todayBookings.filter(
    (b) => b.labStatus === 'report_ready'
  )

  const { data: pendingSettlement } = useSWR(
    ...
    <>
            ...
            <div className="db-lab-inner">
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  <Badge
                    variant={lab.isApproved ? 'success' : 'warning'}
                    size="sm"
                    dot
                  >
                  ...
                  <Badge
                    variant={lab.isActive ? 'success' : 'neutral'}
                    size="sm"
                  >
      ...
      <div style={{ marginBottom: 20 }}>
        <SCard
          ...
          action={
            <ViewAllLink onClick={() => router.push('/lab-admin/bookings')} />
          }
        >
          {allLoading ? (
