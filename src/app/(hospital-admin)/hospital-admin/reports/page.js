    ...
    <div style={{ background:'#fff', border:'1px solid #f1f5f9', borderRadius:12, padding:'10px 14px', boxShadow:'0 4px 16px rgba(0,0,0,0.1)', fontSize:12 }}>
      ...
      {payload.map((e) => (
        <p key={e.name} style={{ color:e.color, margin:'2px 0' }}>
          {e.name}: {e.name==='Gross Revenue'?`₹${Number(e.value).toLocaleString('en-IN')}`:e.value}
        </p>
      ))}
    </div>
  )
...
export default function HospitalReportsPage() {
  ...
  const chartData = (revenueData?.chartData||[]).map((d) => ({
    date:d.date, 'Gross Revenue':Math.round(d.revenue||0), Bookings:d.bookings||0,
  }))

  return (
    <>
      <AdminHeader title="Reports" subtitle="Hospital booking and revenue analytics"
            ...
            <div style={{ height:260 }}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{top:5,right:20,bottom:5,left:0}}>
                  ...
                  <Legend />
                  <Line type="monotone" dataKey="Gross Revenue" stroke="#6366f1" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </SCard>
