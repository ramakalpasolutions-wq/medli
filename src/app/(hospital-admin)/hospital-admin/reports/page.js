// src/app/(hospital-admin)/hospital-admin/reports/page.js
'use client'
    ...
    <div className="bg-white border border-gray-100 rounded-xl p-3 shadow-lg text-xs">
      ...
      {payload.map((entry) => (
        <p key={entry.name} style={{ color: entry.color }}>
          {entry.name}: {entry.name === 'Gross Revenue'
...
export default function HospitalReportsPage() {
  ...
  // ✅ Show gross revenue (what hospital earns before MEDLI fee) — NOT platform fee
  // grossRevenue = totalRevenue - platformFee - gst (what hospital actually receives)
  const grossRevenue = (s.totalRevenue || 0)
  const netRevenue   = grossRevenue - (s.totalPlatformFee || 0) - (s.totalGst || 0)

  const chartData = (revenueData?.chartData || []).map((d) => ({
    date:           d.date,
    'Gross Revenue': Math.round(d.revenue     || 0),
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <StatsCard
            title="Gross Revenue"
            value={`Rs. ${Number(grossRevenue).toLocaleString('en-IN')}`}
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                <Legend />
                <Line type="monotone" dataKey="Gross Revenue" stroke="#1286f5" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Card>
