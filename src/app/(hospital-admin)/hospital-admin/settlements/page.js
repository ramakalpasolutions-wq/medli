// src/app/(hospital-admin)/hospital-admin/settlements/page.js
'use client'
...
export default function HospitalSettlements() {
  ...
  const handleDownload = async (settlementId, settlementNumber) => {
    ...
    try {
      ...
      if (!res.ok) { const err = await res.json().catch(() => ({})); toast.error(err.error || 'Download failed'); return }
  ...
  const cols = [
    { key: 'settlementNumber', header: 'Settlement #', render: (v) => <span className="font-mono text-xs font-bold text-gray-700">{v}</span> },
    { key: 'totalBookings',    header: 'Bookings',     render: (v) => <span className="text-sm font-medium text-gray-800">{v || 0}</span> },
    { key: 'grossAmount',      header: 'Revenue',      render: (v) => <span className="text-sm text-gray-700">Rs. {Number(v || 0).toLocaleString('en-IN')}</span> },
    { key: 'refundsDeducted',  header: 'Refunds',      render: (v) => v > 0 ? <span className="text-sm text-red-500">- Rs. {Number(v).toLocaleString('en-IN')}</span> : <span className="text-xs text-gray-400">None</span> },
    { key: 'netSettlementAmount', header: 'Transferred', render: (v) => <span className="text-sm font-bold text-emerald-600">Rs. {Number(v || 0).toLocaleString('en-IN')}</span> },
    { key: 'status',           header: 'Status',       render: (v) => <Badge variant={getStatusVariant(v)} size="sm" dot>{v}</Badge> },
    { key: 'utrNumber',        header: 'UTR',          render: (v) => v ? <span className="font-mono text-xs text-gray-700">{v}</span> : <span className="text-xs text-gray-400">—</span> },
    { key: 'transferredAt',    header: 'Date',         render: (v) => mounted && v ? new Date(v).toLocaleDateString('en-IN', { dateStyle: 'medium' }) : '—' },
    {
      key: 'actions', header: '',
      render: (_, row) => (
        <button onClick={() => handleDownload(row.id, row.settlementNumber)} disabled={dlId === row.id}
          ...
          {dlId === row.id ? <span className="w-3 h-3 border-2 border-blue-400 border-t-transparent rounded-full animate-spin" /> : <Download className="w-3.5 h-3.5" />}
          PDF
        </button>
      ),
    },
  ...
  return (
    <div>
      <AdminHeader
        ...
        actions={<Button size="sm" variant="secondary" leftIcon={<Building2 className="w-3.5 h-3.5" />} onClick={() => setBankOpen(true)}>Update Bank Account</Button>}
      />
      ...
      <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-4 mb-5">
        ...
        <p className="text-sm text-blue-700">
          You receive the full consultation fee paid by patients.
          Only refunds from cancelled bookings are deducted.
        </p>
      </div>

      {/* ── Pending settlement card (matches lab design) ── */}
      {myPending ? (
        <motion.div
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl p-6 mb-6"
        ...
        >
          ...
          {/* ✅ Exactly matching lab design — 3 cards */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            <div className="bg-white/70 rounded-xl p-4">
              <p className="text-xs text-gray-500 mb-1">Consultation Revenue</p>
      ...
      <Modal open={bankOpen} onClose={() => setBankOpen(false)} title="Update Bank Account" size="md">
        <p className="text-sm text-gray-500 mb-4">Settlement amounts will be transferred directly to this account.</p>
        <div className="space-y-3">
          <Input label="Account Holder Name *" value={bankForm.accountHolderName} onChange={(e) => setBankForm((f) => ({ ...f, accountHolderName: e.target.value }))} placeholder="As per bank records" />
          <Input label="Account Number *"       value={bankForm.accountNumber}      onChange={(e) => setBankForm((f) => ({ ...f, accountNumber: e.target.value }))} />
          <div className="grid grid-cols-2 gap-3">
            <Input label="IFSC Code *" value={bankForm.ifscCode} onChange={(e) => setBankForm((f) => ({ ...f, ifscCode: e.target.value.toUpperCase() }))} placeholder="SBIN0001234" />
            <Input label="Bank Name"   value={bankForm.bankName} onChange={(e) => setBankForm((f) => ({ ...f, bankName: e.target.value }))} />
          </div>
          <Input label="UPI ID (Optional)" value={bankForm.upiId} onChange={(e) => setBankForm((f) => ({ ...f, upiId: e.target.value }))} placeholder="name@upi" />
        </div>
