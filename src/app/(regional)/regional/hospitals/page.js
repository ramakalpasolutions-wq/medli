...
export default function RegionalHospitals() {
  ...
  const { data, isLoading } = useSWR(`/api/hospitals?page=${page}&limit=20`, fetcher)

  const columns = [
    { key: 'name', header: 'Hospital', render: (v) => <span className="font-medium">{v}</span> },
  ...
  return (
    <div>
      <AdminHeader title="Hospitals" subtitle="Region hospitals (read-only)" />
