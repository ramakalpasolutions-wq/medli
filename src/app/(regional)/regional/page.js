...
export default function RegionalDashboard() {
  ...
  return (
    <div>
      <AdminHeader title={`Welcome, ${user?.name?.split(' ')[0] || 'Manager'}`} subtitle="Regional dashboard" />
      ...
      <Card title="Recent Bookings">
        <DataTable columns={bookingCols} data={bookings?.bookings || []} emptyTitle="No recent bookings" />
      </Card>
    </div>
  )
}